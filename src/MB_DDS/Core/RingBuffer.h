...
struct alignas(64) SubscriberState {
    ...
    std::atomic<uint64_t> next_expected_sequence; ///< 下一个期望读取的序列号
    std::atomic<bool> active;                    ///< 订阅者是否活跃
    uint64_t subscriber_id;                      ///< 订阅者唯一标识符
    
    SubscriberState() : last_read_sequence(0), next_expected_sequence(1), 
                       active(false), subscriber_id(0) {}
};
...
public:
     ...
     * @param size 缓冲区总大小
     * @param sem 共享内存信号量（用于订阅者注册保护）
     */
    RingBuffer(void* buffer, size_t size, sem_t* sem);
    
    /**
     * @brief 发布消息到环形缓冲区
     * @param message 要发布的消息（包含完整的头部和数据指针）
     * @return 发布成功返回true，缓冲区满时返回false
     */
    bool publish_message(const Message& message);
    
    /**
     * @brief 订阅者读取下一条消息
     * @param current_sequence 订阅者当前已读到的消息序号
     * @param current_position 订阅者当前已读消息在缓冲区中的位置
     * @param out_message 输出参数，返回读取到的消息
     * @param out_sequence 输出参数，返回读取到的消息序号
     * @param out_position 输出参数，返回读取到的消息在缓冲区中的位置
     * @return 读取成功返回true，无新消息返回false
     */
    bool read_next(uint64_t current_sequence, size_t current_position, Message*& out_message, uint64_t& out_sequence, size_t& out_position);
     ...
     */
    bool read_latest(uint64_t current_sequence, Message*& out_message, uint64_t& out_sequence, size_t& out_position);
     ...
     * @return 未读消息数量
     */
    uint64_t get_unread_count(uint64_t current_sequence);
     ...
     */
    bool register_subscriber(uint64_t subscriber_id);
    
    /**
     * @brief 注销订阅者（使用信号量保护）
     * @param subscriber_id 订阅者唯一标识符
     */
    void unregister_subscriber(uint64_t subscriber_id);
     ...
     * @return 跳过的损坏消息数量
     */
    uint32_t skip_corrupted_messages(uint64_t subscriber_id);
     ...
     */
    bool wait_for_message(uint64_t subscriber_id, uint32_t timeout_ms = 0);
    ...
    struct Statistics {
        ...
        uint32_t corrupted_messages;  ///< 损坏消息数
    };
...
private:
    ...
    struct alignas(64) Header {
        ...
        std::atomic<uint32_t> corrupted_count;    ///< 损坏消息计数
        ...
        Header() : write_pos(0), current_sequence(0), notification_count(0),
                  corrupted_count(0), capacity(0), data_offset(0), magic_number(MAGIC) {}
    };
